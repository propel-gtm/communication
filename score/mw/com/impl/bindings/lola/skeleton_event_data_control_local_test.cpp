...
class SkeletonEventDataControlLocalFixture : public ::testing::Test
{
  public:
    ...
    SkeletonEventDataControlLocalFixture& GivenARealSkeletonEventDataControlLocal(
        ...
        const LolaEventInstanceDeployment::SubscriberCountType max_subscribers)
    {
        ...
        unit_ = std::make_unique<SkeletonEventDataControlLocal<>>(*event_data_control_);

        return *this;
    }

    SkeletonEventDataControlLocalFixture& GivenAMockedEventDataControl(
        const SlotIndexType max_slots,
        const LolaEventInstanceDeployment::SubscriberCountType max_subscribers)
    {
        event_data_control_ = std::make_unique<EventDataControl>(max_slots, memory_, max_subscribers);
    ...
    std::unique_ptr<SkeletonEventDataControlLocal<>> unit_{nullptr};
    std::unique_ptr<SkeletonEventDataControlLocal<memory::shared::AtomicIndirectorMock>> unit_mock_{nullptr};
};

TEST_F(SkeletonEventDataControlLocalFixture, CanAllocateOneSlotWithoutContention)
{
    ...
    GivenARealSkeletonEventDataControlLocal(kMaxSlots, kMaxSubscribers);
...
TEST_F(SkeletonEventDataControlLocalFixture, CanAllocateOneSlotWhenReferenceCountChanges)
{
    RecordProperty("Verifies", "SCR-5899076");
    RecordProperty("Description", "Ensures that a slot can be allocated");
    RecordProperty("TestType", "Requirements-based test");
...
TEST_F(SkeletonEventDataControlLocalFixture, CanAllocateMultipleSlotWithoutContention)
{
...
TEST_F(SkeletonEventDataControlLocalFixture, DiscardedElementOnWritingWillBeInvalid)
{
    // Given an initialized EventDataControl structure where already a slot is allocated
    GivenARealSkeletonEventDataControlLocal(kMaxSlots, kMaxSubscribers);
    auto slot = unit_->AllocateNextSlot();
...
TEST_F(SkeletonEventDataControlLocalFixture, DiscardedElementAfterWritingIsNotTouched)
{
...
TEST_F(SkeletonEventDataControlLocalFixture, CanNotAllocateSlotIfAllSlotsAllocated)
{
...
TEST_F(SkeletonEventDataControlLocalFixture, CanAllocateSlotAfterOneSlotReady)
{
    // Given an initialized EventDataControl structure where all slots are allocated
    GivenARealSkeletonEventDataControlLocal(kMaxSlots, kMaxSubscribers);
...
TEST_F(SkeletonEventDataControlLocalFixture, CanAllocateOldestSlotAfterOneSlotReady)
{
    // Given an initialized EventDataControl structure where all slots are allocated
    GivenARealSkeletonEventDataControlLocal(kMaxSlots, kMaxSubscribers);
    std::array<ControlSlotIndicator, 5U> slot_indicators{};
...
TEST_F(SkeletonEventDataControlLocalFixture, MultithreadedSlotAllocationDeallocation)
{
...
using EventDataControlReferenceSpecificEventFixture = SkeletonEventDataControlLocalFixture;
TEST_F(EventDataControlReferenceSpecificEventFixture, ReferenceSpecificEvents)
{
    ...
    GivenARealSkeletonEventDataControlLocal(max_number_slots, kMaxSubscribers);
...
TEST_F(EventDataControlReferenceSpecificEventDeathTest, ReferenceSpecificEvent_StatusInvalidTerminates)
{
    ...
    GivenARealSkeletonEventDataControlLocal(1, kMaxSubscribers);
...
TEST_F(SkeletonEventDataControlLocalFixture, AllocatedSlotsCanBeCleanedUp)
{
...
using EventDataControlDeathTest = SkeletonEventDataControlLocalFixture;

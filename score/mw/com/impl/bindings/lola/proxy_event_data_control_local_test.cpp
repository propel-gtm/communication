...
class ProxyEventDataControlLocalFixture : public ::testing::Test
{
  public:
    ...
    ProxyEventDataControlLocalFixture& GivenARealProxyEventDataControlLocal(
        ...
        const LolaEventInstanceDeployment::SubscriberCountType max_subscribers)
    {
        ...
        unit_ = std::make_unique<ProxyEventDataControlLocal<>>(*event_data_control_);
        skeleton_event_data_control_local_ = std::make_unique<SkeletonEventDataControlLocal<>>(*event_data_control_);

        return *this;
    }

    ProxyEventDataControlLocalFixture& GivenAMockedEventDataControl(
        const SlotIndexType max_slots,
        const LolaEventInstanceDeployment::SubscriberCountType max_subscribers)
    {
        event_data_control_ = std::make_unique<EventDataControl>(max_slots, memory_, max_subscribers);
        ...
        unit_mock_ =
            std::make_unique<ProxyEventDataControlLocal<memory::shared::AtomicIndirectorMock>>(*event_data_control_);
        skeleton_event_data_control_local_mocked_ =
            std::make_unique<SkeletonEventDataControlLocal<memory::shared::AtomicIndirectorMock>>(*event_data_control_);
    ...
    std::unique_ptr<SkeletonEventDataControlLocal<>> skeleton_event_data_control_local_{nullptr};
    std::unique_ptr<SkeletonEventDataControlLocal<memory::shared::AtomicIndirectorMock>>
        skeleton_event_data_control_local_mocked_{nullptr};
    std::unique_ptr<ProxyEventDataControlLocal<>> unit_{nullptr};
    std::unique_ptr<ProxyEventDataControlLocal<memory::shared::AtomicIndirectorMock>> unit_mock_{nullptr};
};

TEST_F(ProxyEventDataControlLocalFixture, RegisterProxyElementReturnsValidTransactionLogIndex)
{
...
TEST_F(ProxyEventDataControlLocalFixture, FindNextSlotBlocksAllocation)
{
    // Given a EventDataControlUnit with one ready slot
    GivenARealProxyEventDataControlLocal(1, kMaxSubscribers).WithAnAllocatedSlot(1);
...
TEST_F(ProxyEventDataControlLocalFixture, DISABLED_MultipleReceiverRefCountCheck)
{
...
TEST_F(ProxyEventDataControlLocalFixture, FailingToUpdateSlotValueCausesReferenceNextEventToReturnNull)
{
...
TEST_F(ProxyEventDataControlLocalFixture, GetNumNewEvents_One)
{
    // Given an EventDataControl with one ready slot
    GivenARealProxyEventDataControlLocal(1, kMaxSubscribers).WithAnAllocatedSlot(1);
...
TEST_F(ProxyEventDataControlLocalFixture, GetNumNewEvents_Many)
{
    // Given an EventDataControl with 6 ready slots
    GivenARealProxyEventDataControlLocal(6, kMaxSubscribers);
...
class MultiSenderMultiReceiverTest : public ::testing::TestWithParam<MultiSenderMultiReceiverParams>
{
  public:
    ...
    {
        ProxyEventDataControlLocal<>::ResetPerformanceCounters();
    }
    ~MultiSenderMultiReceiverTest() override
    {
        ProxyEventDataControlLocal<>::DumpPerformanceCounters();
    }
  ...
  protected:
    ...
    SkeletonEventDataControlLocal<> skeleton_event_data_control_local_{event_data_control_};
    ProxyEventDataControlLocal<> unit_{event_data_control_};
};
